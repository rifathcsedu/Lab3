...
if __name__ == '__main__':
    ...
    link_layer.add_link(link.Link(client2, 0, router_a, 0, 40))
    #Router A
    link_layer.add_link(link.Link(router_a, 0, router_b, 0, 30))
    link_layer.add_link(link.Link(router_a, 0, router_c, 0, 50))
    #router b
    link_layer.add_link(link.Link(router_b, 0, router_d, 0, 40))
    #router c
    link_layer.add_link(link.Link(router_c, 0, router_d, 0, 60))
    #router d
    link_layer.add_link(link.Link(router_d, 0, server1, 0, 40))
    ...
    str1='I am expecting a lot of transfer activity from last season top six.(give the network).give the network sufficient time to transfer all packets before quitting'
    str2='The current router implementation supports very simple forwarding.  The router has only one input and one output interface and just forwards packets from one to the other.  Your tasks is to implement forwarding of packets within routers based on routing tables.'
    
    #create some send events
    #print(len(str))
    client1.udt_send(3, str1)
    #sleep(5)
    #client1.stop=true
    #client2.udt_send(4,str2)
